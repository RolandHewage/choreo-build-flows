...
#
# VERSION: 0.12.0 — added GOOGLE_RUNTIME_IMAGE_REGION=us to match production
# build flow (workflow-utils.ts line 277). This causes the buildpack to
# download SDKs from us-docker.pkg.dev instead of dl.google.com.
...
echo "  v0.12.0 (GOOGLE_RUNTIME_IMAGE_REGION=us)"
echo "========================================"
...
BUILD_CMD="pack build $IMAGE $DOCKER_HOST_FLAG --builder \"$_BUILDER_IMAGE\" --run-image=\"$_RUN_IMAGE\" --path $fullPath --env GOOGLE_RUNTIME_IMAGE_REGION=us $_LANG_ENV $_LANG_VOLUMES $_MAVEN_BINDING --pull-policy if-not-present --trust-builder"

# Angular NPM Webapp Proxy E2E Test — Podman Build Flow

E2E test image that runs a real `podman build` for an Angular webapp, validating the
...
validates the same proxy code paths with Angular-specific dependencies (`@angular/core`).

## Build & push

```bash
cd docker/
docker build --platform linux/amd64 -t rolandhewage/webapp-angular-npm-proxy-e2e:0.1.0 .
docker push rolandhewage/webapp-angular-npm-proxy-e2e:0.1.0
```
...
```bash
kubectl run angular-test --rm -it --restart=Never \
  --image=rolandhewage/webapp-angular-npm-proxy-e2e:0.1.0 \
  --overrides='{
    "spec":{
      "containers":[{
        "name":"angular-test",
...
|---|---|---|
| Dependencies | `react`, `react-dom` | `@angular/core`, `@angular/common`, `rxjs`, `zone.js` |
...
| 2 | Proxy without auth (real Nexus via ngrok) | PASSED (note: ran after scenario 3, packages cached in Nexus blob store) |
...
E2E image: `rolandhewage/webapp-angular-npm-proxy-e2e:0.1.0`
